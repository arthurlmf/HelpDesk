...
public abstract class AbstractDAO {
	 ...
	 */
	private AbstractDAO(SessionFactory sessionFactory) {// never used
	 ...
	 */
	protected synchronized Serializable insert(Object obj)
		...
		try {
			...
			if (tx != null)
				tx.rollback();
			throw new HelpDeskException(MsgErros.OPER_NAO_REALIZADA.msg(
					"Inserção", e.getMessage()));
		} finally {
			session.flush();
			session.close();
		}
	 ...
	 *         queryString.
	 */
	protected synchronized List getList(String conditionQueryString,
			String nameClass) {
		Session session = openSession();
		String queryCompleta = "from " + nameClass + " objeto where "
	 ...
	 *             caso ocorra algum erro
	 */
	protected synchronized void removeAll(String deleteFrom)
			throws HelpDeskException {
		Session session = openSession();
		Transaction tx = null;
		try {
			...
			Query queryDelete = session
					.createQuery("delete from " + deleteFrom);
			...
			if (tx != null)
				tx.rollback();
			throw new HelpDeskException(MsgErros.OPER_NAO_REALIZADA.msg(
					"Limpeza de cadastro", e.getMessage()));
		} finally {
			session.flush();
			session.close();
		}
	...
	protected synchronized void uptadeQuery(String uptade, String conditionQueryString,
			String nameClass) throws HelpDeskException {
	
		String queryCompleta = "update " + nameClass + " set " + uptade
		...
		try {
			...
			// String queryCompleta = "update tipo.Tipo t set
			// t.chaveTipo.nomeTipo = 'Hidraulico' where t.chaveTipo.nomeTipo =
			// 'Mecanico'";
			Query query = session.createQuery(queryCompleta);
